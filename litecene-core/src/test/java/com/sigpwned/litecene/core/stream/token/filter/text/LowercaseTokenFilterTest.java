...
public class LowercaseTokenFilterTest {
  ...
  public void shouldReplaceNonPrintableAsciiInTextTokens() {
    ...
    TokenStream ts = new LowercaseTokenFilter(source);

    List<Token> tokens = TokenStreams.toList(ts);

    assertThat(tokens,
        is(asList(new TextToken(asList(Term.fromString("füñkÿ")), OptionalInt.empty()))));
  }
  ...
  public void shouldNotTouchNonTextTokens() {
    ...
    assertThat(tokens, is(asList(Token.AND, Token.LPAREN, Token.OR, Token.NOT, Token.RPAREN,
        new TextToken(asList(Term.fromString("füñkÿ")), OptionalInt.empty()))));
  }
}
